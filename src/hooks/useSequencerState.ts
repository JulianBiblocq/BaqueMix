...
export function useSequencerState() {
  ...
  // Keep refs in sync
  useEffect(() => {
    // 🛡️ FIX (Audit): Subscribe to Zustand store instead of using reactive hooks to avoid re-renders
  ...
  useEffect(() => {
    const totalMeasures = totalMeasuresRef.current;
  ...
  const handleSetLoopStart = (measureIdx: number) => {
    ...
    if (loopEndRef.current !== null && measureIdx > loopEndRef.current) {
  ...
  const handleSetLoopEnd = (measureIdx: number) => {
    ...
    setLoopEndMeasure(measureIdx);
    setIsLoopRegionActive(true);
    if (loopStartRef.current !== null && measureIdx < loopStartRef.current) {
      setLoopStartMeasure(measureIdx);
    }
  };
  ...
  return {
    ...
    setTotalMeasures, totalMeasuresRef,
    timeSig, setTimeSig,
    setMeasureTimeSigs, measureTimeSigsRef,
